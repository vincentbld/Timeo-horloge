...
void loop() {
  ...
  // Convertir en format 12 heures (1 à 12)
  int hour12Format = currentHour % 12;
  if (hour12Format == 0) {
    hour12Format = 12; // Minuit et midi sont représentés par 12
  }

  // Calculer la position des pixels pour l'heure actuelle
  // Inverser de 180 degrés en décalant la position de départ de 26 pixels (la moitié de 52)
  int pixelIndex = (map(hour12Format * 60 + currentMinute, 0, 12 * 60, 0, 52) + 26) % 52;
  ...
  if (currentHour >= 6 && currentHour < 12) {
    ...
    currentColor = nightColor;  // Nuit, LEDs en bleu profond
  }
